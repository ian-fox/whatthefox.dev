...
{{ shell(command="gcc hello.c -o hello", dialect="sh") }}
{% shell(command="./hello", dialect="sh") %}
Hello, world!
{% end %}
...
{{ shell(command='xxd -p -c 1 hello | xargs printf "\\\\x%s" > hello.hex', dialect="sh") }}
...
{{ shell(command='xxd -p -c 1 hello | xargs printf "\\\\x%s" | fold > hello.hex', dialect="sh") }}
...
{{ shell(command="echo -e 'pasted contents of hello.hex' > hello_pasted", dialect="sh") }}
{% shell(command="diff hello hello_pasted", dialect="sh") %}
...
{{ shell(command="xxd hello > hello.hex", dialect="sh") }}
{{ shell(command="xxd hello_pasted > hello_pasted.hex", dialect="sh") }}
{% shell(command="diff -y hello.hex hello_pasted.hex | head", dialect="sh") %}
...
{{ shell(command=`xxd -p -c 1 hello.c.out | xargs printf "\\\\\\\\x%s" | fold | xargs printf "echo -e -n '%s' >> /tmp/hello\n" > commands.sh1`, dialect="sh") }}
